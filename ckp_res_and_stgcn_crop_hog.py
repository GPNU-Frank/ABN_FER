...
from models import RensnetAndSTGCNAndCrop, RensnetAndSTGCNAndCropHOG
import numpy as np
from utils import Bar, Logger, AverageMeter, accuracy, mkdir_p, savefig, pickle_2_img_single, pickle_2_img_and_landmark_and_crop
import logging
...
# Architecture
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet_and_stgcn_and_crop_hog')
...
def main():
    ...
    features, landmarks, leyes, reyes, noses, lips, labels = pickle_2_img_and_landmark_and_crop(args.dataset_path)
    num_classes = 6

    # Model
    print("==> creating model '{}'".format(args.arch))
    model = RensnetAndSTGCNAndCropHOG(num_classes=num_classes)
    ...
    for f_num in range(args.folds):
        ...
        train_x, train_lm, train_leye, train_reye, train_nose, train_lip, train_y = [], [], [], [], [], [], []
        test_x, test_lm, test_leye, test_reye, test_nose, test_lip, test_y = [], [], [], [], [], [], []
        for id_fold in range(args.folds):
            if id_fold == f_num:
                test_x = features[id_fold]
                test_lm = landmarks[id_fold]
                test_leye = leyes[id_fold]
                test_reye = reyes[id_fold]
                test_nose = noses[id_fold]
                test_lip = lips[id_fold]
                test_y = labels[id_fold]
            else:
                train_x = train_x + features[id_fold]
                train_lm = train_lm + landmarks[id_fold]
                train_leye = train_leye + leyes[id_fold]
                train_reye = train_reye + reyes[id_fold]
                train_nose = train_nose + noses[id_fold]
                train_lip = train_lip + lips[id_fold]
        ...
        train_leye = torch.tensor(train_leye, dtype=torch.float) / 255.0  #(b_s, 1, 24, 32)
        train_leye = train_leye.unsqueeze(1)

        train_reye = torch.tensor(train_reye, dtype=torch.float) / 255.0  #(b_s, 1, 24, 32)
        train_reye = train_reye.unsqueeze(1)

        train_nose = torch.tensor(train_nose, dtype=torch.float) / 255.0  #(b_s, 1, 32, 24)
        train_nose = train_nose.unsqueeze(1)

        train_lip = torch.tensor(train_lip, dtype=torch.float) / 255.0  #(b_s, 1, 24, 32)
        train_lip = train_lip.unsqueeze(1)
        ...
        test_leye = torch.tensor(test_leye, dtype=torch.float) / 255.0  #(b_s, 1, 24, 32)
        test_leye = test_leye.unsqueeze(1)
        test_reye = torch.tensor(test_reye, dtype=torch.float) / 255.0  #(b_s, 1, 24, 32)
        test_reye = test_reye.unsqueeze(1)
        test_nose = torch.tensor(test_nose, dtype=torch.float) / 255.0  #(b_s, 1, 32, 24)
        test_nose = test_nose.unsqueeze(1)
        test_lip = torch.tensor(test_lip, dtype=torch.float) / 255.0  #(b_s, 1, 24, 32)
        test_lip = test_lip.unsqueeze(1)
        ...
        train_dataset = torch.utils.data.TensorDataset(train_x, train_lm, train_leye, train_reye, train_nose, train_lip, train_y)
        ...
        test_dataset = torch.utils.data.TensorDataset(test_x, test_lm, test_leye, test_reye, test_nose, test_lip, test_y)
...
def train(train_iter, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, landmarks, train_leye, train_reye, train_nose, train_lip, targets) in enumerate(train_iter):
        # measure data loading time
        data_time.update(time.time() - end)

        if use_cuda:
            inputs, landmarks, train_leye, train_reye, train_nose, train_lip, targets = inputs.cuda(), landmarks.cuda(), train_leye.cuda(), train_reye.cuda(), train_nose.cuda(), train_lip.cuda(), targets.cuda()
        # inputs, targets = torch.autograd.Variable(inputs), torch.autograd.Variable(targets)

        # compute output
        per_outputs = model((inputs, landmarks, train_leye, train_reye, train_nose, train_lip))
...
def test(test_iter, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, landmarks, test_leye, test_reye, test_nose, test_lip, targets) in enumerate(test_iter):
    # measure data loading time
        data_time.update(time.time() - end)

        if use_cuda:
            inputs, landmarks, test_leye, test_reye, test_nose, test_lip, targets = inputs.cuda(), landmarks.cuda(), test_leye.cuda(), test_reye.cuda(), test_nose.cuda(), test_lip.cuda(), targets.cuda()
        # inputs, targets = torch.autograd.Variable(inputs, volatile=True), torch.autograd.Variable(targets)

        # compute output
        outputs = model((inputs, landmarks, test_leye, test_reye, test_nose, test_lip))
...
if __name__ == '__main__':
    ...
    # crop1 = torch.rand((32, 1, 24, 32), dtype=torch.float)
    # crop2 = torch.rand((32, 1, 24, 32), dtype=torch.float)
    # crop3 = torch.rand((32, 1, 32, 24), dtype=torch.float)
    # crop4 = torch.rand((32, 1, 24, 32), dtype=torch.float)
    # lm = torch.rand((32, 2, 1, 55), dtype=torch.float)
    # model = RensnetAndSTGCNAndCropHOG(num_classes=6)
    # out = model((img, lm, crop1, crop2, crop3, crop4))
    # print(model)
    # print(out.size())
    main()
