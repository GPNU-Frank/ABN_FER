...
from models import ResNetAndGCN, GCNPair
import numpy as np
...
parser.add_argument('--dataset-path', default='data\ck+_6_classes_img_and_55_landmark_106_pair.pickle')  # windows style
...
# Checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/ckp_gcn_pair', type=str, metavar='PATH',
                    help='path to save checkpoint (default: checkpoint)')
parser.add_argument('--resume', default='', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
# Architecture
parser.add_argument('--arch', '-a', metavar='ARCH', default='gcn_pair')
...
def main():
    ...
    for f_num in range(args.folds):
        ...
        # 只要坐标信息， 不需要归一化
        train_lm = (train_lm - np.amin(train_lm)) / np.amax(train_lm) - np.amin(train_lm)
        ...
        test_lm = (test_lm - np.amin(test_lm)) / np.amax(test_lm) - np.amin(test_lm)
