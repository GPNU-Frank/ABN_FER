...
from models import RensnetAndSTGCNAndCrop
import numpy as np
...
# Architecture
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet_and_stgcn_and_crop')
...
def main():
    ...
    features, landmarks, crops, labels = pickle_2_img_and_landmark_and_crop(args.dataset_path)
    num_classes = 6

    # Model
    print("==> creating model '{}'".format(args.arch))
    model = RensnetAndSTGCNAndCrop(num_classes=num_classes)
    ...
    for f_num in range(args.folds):
        ...
        train_crop = torch.tensor(train_crop, dtype=torch.float) / 255.0  #(b_s, 4, 24, 32)
        ...
        test_crop = torch.tensor(test_crop, dtype=torch.float) / 255.0
        ...
        train_dataset = torch.utils.data.TensorDataset(train_x, train_lm, train_crop, train_y)
        ...
        test_dataset = torch.utils.data.TensorDataset(test_x, test_lm, test_crop, test_y)
...
def train(train_iter, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, landmarks, crops, targets) in enumerate(train_iter):
        # measure data loading time
        data_time.update(time.time() - end)

        if use_cuda:
            inputs, landmarks, crops, targets = inputs.cuda(), landmarks.cuda(), crops.cuda(), targets.cuda()
        # inputs, targets = torch.autograd.Variable(inputs), torch.autograd.Variable(targets)

        # compute output
        per_outputs = model((inputs, landmarks, crops))
...
def test(test_iter, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, landmarks, crops, targets) in enumerate(test_iter):
    # measure data loading time
        data_time.update(time.time() - end)
        ...
        # compute output
        outputs = model((inputs, landmarks, crops))
